...
> 后端提供 **Express (Node.js)** 与 **Spring Boot (Java)** 两种实现，REST API 与行为已按同一份契约对齐，可按技术栈偏好选择部署（仍存在的差异见 [双后端差异](#双后端差异)）。当前已迭代至「精细化体验期」，含 PWA、骨架屏、错误边界、灯箱预览、主题色体系、移动端深度适配等工程化特性。
...
### 博客前台
- **路由/渲染**：`/` 欢迎落地页（SSR）、`/home` 主站、归档/分类/标签/关于页均为**实时 SSR**（不做服务端页面缓存）、工具箱纯客户端渲染
