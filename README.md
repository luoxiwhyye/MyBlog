...
> 后端提供 **Express (Node.js)** 与 **Spring Boot (Java)** 两种实现，**功能完全对齐**，可按技术栈偏好选择部署。当前已迭代至「精细化体验期」，含 PWA、ISR、骨架屏、错误边界、灯箱预览、主题色体系、移动端深度适配等工程化特性。
...
- **页面**：首页（公告栏 + 博主信息卡 + 3/2/1 列网格）、文章、分类、标签、归档（搜索/筛选/排序）、关于（个人品牌墙）、友链 `/friends`、留言板、搜索、工具箱、404
- **工具箱**：编解码/格式化/哈希加密/文本处理/颜色工具/开发辅助，含快捷键 (Ctrl/Cmd+K)、历史快照、收藏、防抖、复制反馈
...
├── scripts/                     # initBlogger / regenerateThumbs
...
| `GET`                 | `/dashboard/stats` `/dashboard/charts`         | 仪表盘 / 未读红点            | admin    |
...
- [documents/design-documents/design-system.md](./documents/design-documents/design-system.md) — 设计系统规范（设计 Token）
- [documents/design-documents/anime-restyle-design-spec.md](./documents/design-documents/anime-restyle-design-spec.md) — 二次元重构设计定稿
