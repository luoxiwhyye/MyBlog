...
> 这一节查的全是「**页面外观**」，判定方式统一为：**用真实域名打开页面用眼确认**。
> 原因：这些值都存在数据库里（后台「基本设置」），**没填时页面不报错，只是那一行不渲染** ——
> 服务全绿、冒烟全过也说明不了它们显示正常。

- [ ] **[阻断]** 用真实域名打开前台，**页脚能看到 ICP 备案号**，且与实际备案主体、域名一致
      （含是否带 `www`）。它来自后台「基本设置 · ICP 备案号」，**没填就什么都不显示**。
- [ ] 做过公安联网备案的，后台「基本设置 · 公安联网备案号」也已填：
      前台页脚与后台登录页会同时给出两条备案号 + 公安图标，
      各自链到 `beian.miit.gov.cn` / `beian.mps.gov.cn`
      （号码里带备案编码数字时，链接直接定位到该条备案）。
      该图标是仓库内的静态文件（`myblog-blog/public/police.png`、`myblog-admin/public/police.png`），**不是配置项**。
