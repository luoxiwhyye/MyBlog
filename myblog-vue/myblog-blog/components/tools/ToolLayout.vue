...
const isFavorite = computed(() =>
  currentToolId.value ? favoriteIds.value.includes(currentToolId.value) : false,
)

const toggleFavorite = () => {
  if (!currentToolId.value) return
  const idx = favoriteIds.value.indexOf(currentToolId.value)
  if (idx >= 0) {
    favoriteIds.value.splice(idx, 1)
  } else {
    favoriteIds.value.push(currentToolId.value)
  }
